...
export const Game = {
  ...
  init(canvasEl, uiRefs) {
    ...
    canvasEl.addEventListener('mousedown', e => handleInput(e.clientX, e.clientY));
  ...
  start(size, mode, diff) {
    ...
    // 1. Скриј го Canvas-от ВЕДНАШ за да не се гледа "glitch"
    if (this.renderer.canvas) {
        this.renderer.canvas.style.opacity = '0';
        this.renderer.canvas.style.transition = 'none'; // Моментално криење
    }
    ...
    // Чистење на стилови (како и претходно)
    if(this.renderer.canvas) {
    ...
    setTimeout(() => {
        // 2. Пресметај ги димензиите додека е скриено
        this.renderer.resize(this.ui.viewGame, this.board.S);
        this.updateUI();
        
        // 3. Откако е спремно, прикажи го Canvas-от
        // Мал fade-in ефект за да изгледа мазно
        this.renderer.canvas.style.transition = 'opacity 0.2s ease-out';
  ...
  processInput(clientX, clientY) {
    if (this.gameOver || this.aiThinking || (this.settings.mode === 'ai' && this.player === CONSTANTS.O)) return;

    const rect = this.renderer.canvas.getBoundingClientRect();
    const scaleX = this.renderer.canvas.width / rect.width / (window.devicePixelRatio || 1);
    const scaleY = this.renderer.canvas.height / rect.height / (window.devicePixelRatio || 1);
    ...
    let best = null, minDist = CELL * 0.6;
  ...
  executeMove(move) {
    ...
    if (res.points > 0) {
        audio.score(); Native.vibrate('medium');
  ...
  endGame() {
    this.gameOver = true; audio.win(); Native.vibrate('heavy');
    const s = this.board.score;
    document.getElementById("result-title").textContent = s[CONSTANTS.X] > s[CONSTANTS.O] ? "You Win!" : (s[CONSTANTS.O] > s[CONSTANTS.X] ? "Game Over" : "Draw");
    document.getElementById("result-score").textContent = `${s[CONSTANTS.X]} - ${s[CONSTANTS.O]}`;
  ...
  updateUI() {
    ...
    if(this.player === CONSTANTS.X) { this.ui.p1Card.classList.add("active"); this.ui.p2Card.classList.remove("active"); }
    else { this.ui.p1Card.classList.remove("active"); this.ui.p2Card.classList.add("active"); }
    
    if(this.historyStack.length > 0) this.ui.undoBtn.classList.remove('disabled');
    else this.ui.undoBtn.classList.add('disabled');
  },

  loop() {
    if (this.ui.viewGame.classList.contains('hidden')) return;
    if (this.renderer.CELL <= 0) this.renderer.resize(this.ui.viewGame, this.board.S);
    
    this.renderer.draw(this.board, this.lastMove);
    
    if (!this.gameOver || this.renderer.particles.length > 0) requestAnimationFrame(() => this.loop());
  },
